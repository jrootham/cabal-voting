...
initialModel target =
    let
        ...
        paperModel =
            PaperModel [] Title "" [] Nothing

        userModel =
            UserModel Nothing Nothing
    in
...
makeRules buffer =
    case buffer of
        Just rulesBuffer ->
            let
                result0 =
                    makeMaxPapers (Rules 0 0 0) rulesBuffer

                result1 =
                    makeMaxVotes result0 rulesBuffer

                result =
                    makeMaxPerPaper result1 rulesBuffer
            in
            case result of
                Ok rules ->
...
type alias ClosedPaperList =
    { paperList : List ClosedPaper }
...
type alias OpenPaperList =
    { paperList : List OpenPaper }
