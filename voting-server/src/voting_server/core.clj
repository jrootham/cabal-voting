...
(compojure/defroutes voting
	(compojure/GET "*/servers/voting/register-prompt" [] (register/register-prompt "" "" []))
	(compojure/POST "*/servers/voting/register" [name address] 
	...
	(route/resources (stuff/resources))
	(route/not-found {:status 404 :body (html/page [:div {:style styles/large-print} "Page not found"])})
)
