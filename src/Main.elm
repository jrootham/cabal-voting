...
import AdminPaper exposing (closedList, openList)
...
import Http exposing (Body, Error, emptyBody, expectString, expectStringResponse, request, send)
...
import Payload exposing (loginPayload, paperIdPayload, paperPayload, rulesPayload, userPayload)
import RulesPage exposing (editRules)
import Set
import Types exposing (..)
import User exposing (editUser, userPage)
...
update msg model =
    case msg of
        ...
        Add ->
            case model.currentUser of
                Just user ->
                    let
                        temp =
                            setEdit (Just (newPaper user.name)) model
                    in
        ...
        DoEdit id ->
            let
                temp =
                    setEdit (getPaper (getPaperList model) id) model
            in
            ( { temp | page = Edit }, Cmd.none )
...
putUserList model response =
    case parseUserList response of
        Ok userList ->
            let
                temp =
                    setUserList (Just userList.userList) model
            in
            { temp | page = Users }
...
newPaper submitter =
    Paper 0 "" (Link "Paper" "") "" [] (Time.fromTime 0) submitter []
...
addReference maybePaper =
    case maybePaper of
        Just paper ->
            let
                maybeMax =
                    List.maximum (List.map (\reference -> reference.index) paper.references)
...
deleteReference maybePaper referenceIndex =
    case maybePaper of
        Just paper ->
            let
                trim =
                    \reference -> reference.index /= referenceIndex

                trimmedReferences =
                    List.filter trim paper.references

                setIndex =
                    makeSetIndex referenceIndex

                newReferences =
                    List.map setIndex trimmedReferences
            in
    ...
    case maybePaper of
        Just paper ->
            let
                ...
                newReferences =
                    List.map setLink paper.references
            in
            Just { paper | references = newReferences }
...
makeNewReferenceLink =
    makeNewReference setLinkLink


makeNewReferenceText =
    makeNewReference setLinkText
...
getVoters paperList =
    let
        ...
        insert =
            \vote nameSet -> Set.insert vote.name nameSet

        inner : List Vote -> Set.Set String -> Set.Set String
        inner =
            \voteList nameSet -> List.foldl insert nameSet voteList

        nameSet =
            List.foldl (\paper nameSet -> inner paper.votes nameSet) (Set.fromList []) paperList
    in
    Set.toList nameSet
    ...
    case parsePaperList response of
        Ok paperInput ->
            let
                ...
                temp =
                    setVoterList voterList (setVoter sortVoter (setPaperList paperList model))
            in
...
fetchRules =
    fetch "POST" "rules" True RulesResult emptyBody


fetchLogin =
    fetch "POST" "login" True UpdateLogin


fetchReload =
    fetch "POST" "reload" True FetchResult emptyBody


fetchClose =
    fetch "POST" "close" True FetchResult


fetchVote =
    fetch "POST" "vote" True FetchResult


fetchUnvote =
    fetch "POST" "unvote" True FetchResult


fetchSave =
    fetch "POST" "save" True FetchResult


fetchUsers =
    fetch "POST" "userList" True ListUsers emptyBody


fetchUpdateUser =
    fetch "POST" "updateUser" True ListUsers


fetchOpenList =
    fetch "POST" "openList" True ListOpen emptyBody


fetchAdminClose =
    fetch "POST" "adminClose" True ListOpen


fetchClosedList =
    fetch "POST" "closedList" True ListClosed emptyBody


fetchAdminOpen =
    fetch "POST" "adminUnclose" True ListClosed


fetchEditRules =
    fetch "POST" "rules" True EditRules emptyBody


fetchUpdateRules =
    fetch "POST" "updateRules" True EditRules
