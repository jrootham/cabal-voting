module AdminPaper exposing (closedList, openList)

import Common exposing (flipOrder, normalFlatButton)
...
openListRows model paperList =
    let
        comp =
            \left right -> flipOrder (compare left.totalVotes right.totalVotes)
    in
...
closedListRows model paperList =
    let
        comp =
            \left right -> flipOrder (compare left.closedAt right.closedAt)
    in
