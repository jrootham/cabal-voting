...
import Common exposing (makeLink, normalFlatButton, thinFlatButton)
import Date
import Html exposing (Html, div, h3, h5, input, label, option, select, table, td, text, th, thead, tr)
...
paperListPage model =
    let
        ...
        sumVotes =
            \voteList -> List.sum (List.map .votes voteList)
...
voteTable votes =
    let
        rawDisplayVotes =
            \name votes -> tr [] [ td [ class "vote-name" ] [ text name ], td [] [ text (toString votes) ] ]

        displayVotes =
            \vote -> rawDisplayVotes vote.name vote.votes

        totalVotes =
            \votes -> rawDisplayVotes "Total" (List.sum (List.map .votes votes))
    in
...
displayPaper model paper =
    let
        ...
        thisVoterCount =
            let
                possible =
                    List.head (List.filter testVote paper.votes)
            in
            case possible of
                Just vote ->
                    vote.votes
...
countVotes model =
    let
        ...
        outer : Paper -> Int -> Int
        outer =
            \paper count -> List.foldl inner count (List.filter filter paper.votes)
    in
...
voteLimit model thisVoterCount =
    let
        available =
            getMaxVotes model - countVotes model
    in
...
validateAdd model =
    case model.currentUser of
        Just user ->
            let
                submitterFilter =
                    \paper -> user.name == paper.submitter

                paperCount =
                    List.length (List.filter submitterFilter (getPaperList model))
            in
...
getVotes name voteList =
    let
        entry =
            List.head (List.filter (\vote -> vote.name == name) voteList)
    in
    case entry of
        Just vote ->
            vote.votes
